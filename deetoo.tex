...
\author{ Taewoong~Choi~\IEEEmembership{Student Member,~IEEE,}
         Kyoungyong Lee~\IEEEmembership{Student Member,~IEEE,}
...
%TODO: I'm not sure about c_i and q_i . POB
...
The node that inserts an object records the range used to insert that object.
To update or delete, a message is broadcasted within the same range that
the object originally cached.
As a result, Deetoo guarantees all nodes which are caching the object
recieve the update or deletion message.
\iffalse
...
\begin{eqnarray*}
\lefteqn{P_{miss} <}\\
%\frac{\sqrt{N-Q}(N-Q)^{N-Q} e^{-(N-Q)} e^{-\frac{1}{12(N-Q)}} \sqrt{N-C}(N-C)^{N-C} e^p{-(N-C)} e^{-\frac{1}{12(N-C)}}} {\sqrt{N}N^N e^{-N} e^{-\frac{1}{12N+1}} \sqrt{N-C-Q}(N-C-Q)^{N-C-Q} e^{-N+C+Q} e^{-\frac{1}{12(N-C-Q)+1}}}\\
...
\begin{eqnarray*}
\lefteqn{P_{miss} >}\\
%\frac{\sqrt{N-Q}(N-Q)^{N-Q} e^{-(N-Q)} e^{-\frac{1}{12(N-Q)}} \sqrt{N-C}(N-C)^{N-C} e^p{-(N-C)} e^{-\frac{1}{12(N-C)}}} {\sqrt{N}N^N e^{-N} e^{-\frac{1}{12N}} \sqrt{N-C-Q}(N-C-Q)^{N-C-Q} e^{-N+C+Q} e^{-\frac{1}{12(N-C-Q)}}}\\
...
\iffalse
%POB we don't need this, given we have a space limit

\section{Deetoo Test on Planet-Lab}\label{sec:test}
In this section, we present an implementation of Deetoo in a real P2P environment.
We also compare performance metrics to that of theoretical expectation and simulation results.
We built Deetoo as service on top of Brunet \cite{brunet} and deployed Deetoo-enabled nodes in Planet-Lab \cite{planetlab}. 
In order to explain the system model of Deetoo on PlanetLab deployment, we divide system components into five modules:
Underlying P2P network, network size estimation, task distribution, caching, and querying modules.
\iffalse
%planet-lab
